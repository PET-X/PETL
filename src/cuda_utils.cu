...
cudaError_t equal_gpu(float* dev_lhs, const float* dev_rhs, const int3 N, int whichGPU)
{
    return equal(dev_lhs, dev_rhs, N, whichGPU);
}
...
float* copyProjectionDataToGPU(float* g, parameters* params, int whichGPU)
{
	...
	{
		fprintf(stderr, "cudaMalloc(projection[%d,%d,%d]) failed!\n", params->planogramSet[0]->N_v0, params->planogramSet[0]->N_u1, params->planogramSet[0]->N_u0);
...
bool setProjectionGPUparams(parameters* params, int4& N, float4& T, float4& startVals)
{
    if (params == NULL || params->planogramSet.size() == 0)
    {
    ...
    else
    {
        ...
        startVals.w = plan->v1_0;
        startVals.x = plan->v0_0;
        startVals.y = plan->u1_0;
        startVals.z = plan->u0_0;
